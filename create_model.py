...
def loss_function_mask(y_true, y_pred):
    loss = k.mean(k.square(y_pred * k.cast(y_true > tf.reduce_min(y_true), "float32") - y_true), axis = -1)
#     loss = K.sqrt(K.sum(K.square(y_pred*K.cast(y_true> tf.reduce_min(y_true), "float32") - y_pred)) / K.sum(K.cast(y_true>0, "float32") ))
    return loss
...
model5 = unet()
model5.compile(optimizer = Adam(learning_rate = 1e-3), loss = regularized_loss_masked, metrics = ['MAE',r_squared, adjusted_r_squared])
model5.summary()
